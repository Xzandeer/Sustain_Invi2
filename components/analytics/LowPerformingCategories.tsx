...
interface LowPerformingCategoriesProps {
  rows: LowPerformingRow[]
}

const currency = (value: number) => `\u20b1${value.toLocaleString('en-US')}`

export default function LowPerformingCategories({ rows }: LowPerformingCategoriesProps) {
  return (
    <section className="bg-white rounded-xl shadow-sm p-6 border">
      <h2 className="mb-4 text-xl font-semibold text-slate-900">Low-Performing Categories</h2>
      <div className="overflow-x-auto rounded-lg border">
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-semibold text-slate-700">Category</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-slate-700">Total Sales</th>
